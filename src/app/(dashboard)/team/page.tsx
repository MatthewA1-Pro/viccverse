        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <input
              ...
              className="bg-slate-900/40 border border-slate-800 rounded-lg pl-10 pr-4 py-2 text-sm text-slate-300 placeholder:text-slate-600 focus:outline-none focus:border-primary/50 transition-all w-64"
            />
          </div>
            ...
            <GlassCard className="p-6 relative overflow-hidden group hover:border-primary/30 transition-all">
              ...
              <div className="flex justify-between items-start mb-6 relative z-10">
                <div className="flex items-center gap-4">
                  <div className="relative">
                    <img src={member.avatar} alt={member.name} className="w-14 h-14 rounded-full object-cover border-2 border-slate-800 group-hover:border-primary/50 transition-colors" />
                    <div className={`absolute bottom-0 right-0 w-3.5 h-3.5 rounded-full border-2 border-slate-950 ${
              ...
              <div className="pt-4 border-t border-slate-800/50 flex items-center justify-between relative z-10">
                <div className="flex items-center gap-1.5">
                  <ShieldCheck size={16} className={member.id === 1 ? 'text-secondary' : 'text-slate-600'} />
                  <span className="text-xs font-medium text-slate-400">{member.id === 1 ? 'Admin' : 'Member'}</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="text-xs font-bold text-white">{member.performance}%</div>
